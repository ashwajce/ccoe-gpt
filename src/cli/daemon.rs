...
use ccoegpt::concurrency::TurnGate;
use ccoegpt::config::Config;
use ccoegpt::heartbeat::HeartbeatRunner;
use ccoegpt::memory::MemoryManager;
use ccoegpt::server::Server;
...
pub fn daemonize_and_run(agent_id: &str) -> Result<()> {
    ...
    println!(
        "Starting CCOEGPT daemon in background (agent: {})...",
        agent_id
    );
    ...
    println!("\nUse 'ccoegpt daemon status' to check status");
    println!("Use 'ccoegpt daemon stop' to stop\n");
...
async fn start_daemon(foreground: bool, agent_id: &str) -> Result<()> {
    ...
    println!(
        "Starting CCOEGPT daemon in foreground (agent: {})...",
        agent_id
    );
...
async fn restart_daemon(foreground: bool, agent_id: &str) -> Result<()> {
    ...
    #[cfg(unix)]
    if !foreground {
        println!("\nTo start daemon in background, run: ccoegpt daemon start");
...
fn get_pid_file() -> Result<PathBuf> {
    // Put PID file in state dir (~/.ccoegpt/), not workspace
    let state_dir = ccoegpt::agent::get_state_dir()?;
...
fn get_log_file(retention_days: u32) -> Result<PathBuf> {
    ...
    Ok(logs_dir.join(format!("ccoegpt-{}.log", date)))
}
...
fn prune_old_logs(logs_dir: &std::path::Path, keep_days: i64) {
    ...
    if let Ok(entries) = fs::read_dir(logs_dir) {
        for entry in entries.flatten() {
            ...
            // Match ccoegpt-YYYY-MM-DD.log pattern
            if name_str.starts_with("ccoegpt-") && name_str.ends_with(".log") {
                if let Some(date_part) = name_str
                    .strip_prefix("ccoegpt-")
