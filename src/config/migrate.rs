...
//!
//! Provides best-effort migration from OpenClaw's JSON5 config format to CCOEGPT's TOML format.
...
/// Convert OpenClaw config to CCOEGPT config
    ...
    if let Some(agents) = oc.agents {
        if let Some(defaults) = agents.defaults {
            if let Some(workspace) = defaults.workspace {
                // Convert OpenClaw workspace path to CCOEGPT format
                let workspace = workspace.replace("~/.openclaw/", "~/.ccoegpt/");
