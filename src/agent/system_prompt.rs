//! System prompt builder for CCOEGPT agent
//!
...
pub fn build_system_prompt(params: SystemPromptParams) -> String {
    ...
    // Identity
    lines.push("You are a personal assistant running inside CCOEGPT.".to_string());
    ...
    lines.push("CCOEGPT may send periodic heartbeat polls to check on pending tasks.".to_string());
    lines.push(
...
impl<'a> SystemPromptParams<'a> {
    pub fn new(workspace: &'a Path, model: &'a str) -> Self {
        ...
        Self {
            workspace_dir: workspace.to_str().unwrap_or("~/.ccoegpt/workspace"),
            model,
            tool_names: Vec::new(),
