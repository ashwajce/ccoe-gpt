//! Skills system for CCOEGPT (OpenClaw-compatible)
//!
...
pub enum SkillSource {
    ...
    Workspace,
    /// User-managed skill (~/.ccoegpt/skills/)
    Managed,
...
/// Get the managed skills directory (~/.ccoegpt/skills/)
fn get_managed_skills_dir() -> Option<PathBuf> {
    directories::BaseDirs::new().map(|dirs| dirs.home_dir().join(".ccoegpt").join("skills"))
}
